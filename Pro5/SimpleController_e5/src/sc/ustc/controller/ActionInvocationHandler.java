		...
		for (Element res : resultLists) { // 查找对应结果的处理
			if (result.equals(res.attributeValue("name"))) {
				...
				if ("forward".equals(res.attributeValue("type"))) {
					...
					RequestDispatcher rd = (RequestDispatcher)((HttpServletRequest) args[0]).getRequestDispatcher(toPage);
					try {
						HttpServletRequest hRequest = (HttpServletRequest)args[0];
						hRequest.setAttribute("username", hRequest.getParameter("username"));
